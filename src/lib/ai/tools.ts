import { PRIORITIES, STATUSES, ASSESSMENT_TYPES, UNI_STATUSES, UNI_PRIORITIES } from "@/lib/types";
...
export const TOOL_DECLARATIONS = [
  {
    ...
    parameters: {
      type: "OBJECT",
      properties: {
        title: { ...STR, description: "Short imperative title, e.g. 'Finish physics lab report'." },
        dueDate: { ...STR, description: "YYYY-MM-DD. Omit only if there is genuinely no deadline." },
        dueTime: { ...STR, description: "HH:MM, 24-hour. Only when they named a time." },
        courseId: { ...STR, description: "Course id from the context. Omit if unrelated to a course." },
        lesson: { ...STR, description: "Topic within the course, if one of the course's listed topics fits." },
        priority: enumOf(PRIORITIES, "Default to 'medium' unless urgency is clear."),
        notes: { ...STR, description: "Any detail worth keeping that does not belong in the title." },
      },
      required: ["title"],
    },
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        title: STR,
        dueDate: { ...STR, description: "YYYY-MM-DD, or the string 'none' to clear the deadline." },
        dueTime: STR,
  ...
  {
    name: "create_course",
    description: "Add a course. Only when the student is clearly taking a subject the hub does not know about.",
    parameters: {
      type: "OBJECT",
      properties: {
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        weight: { type: "NUMBER", description: "Percentage weight toward the course grade, if known." },
    ...
    parameters: {
      ...
      properties: {
        lines: {
          type: "ARRAY",
          items: STR,
          description: "Each string becomes one paragraph, in the student's own voice where possible.",
        },
        date: { ...STR, description: "YYYY-MM-DD. Defaults to today." },
      },
      required: ["lines"],
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        path: enumOf(
          ["/", "/tasks", "/courses", "/grades", "/universities", "/weight", "/reflection", "/recommendations", "/settings"],
          "Which page to open.",
        ),
      },
      required: ["path"],
